...
fallback_create_issue() {
  ...
  title="[$phase] $name"
  body=$(printf '## 交付物（loop.sh 兜底派单）\n按路线图条目产出：`%s`\n\n- **执行角色**：`%s`（读 `team/%s.md` 人格与输出规范）\n- **输出路径**：`%s`\n- **基线**：遵循 `AGENTS.md` 设计基线；读 `docs/roadmap.md`、`docs/superpowers/`、已有 `web/src/` 对齐\n- **验收**：文件写入指定路径 + 角色 .md 规定的节/字段齐全；代码类跑 `node --check` 覆盖改动文件\n\n主理人 LLM 本轮漏派，由 loop.sh 确定性补建。' "$line" "$role" "$role" "$path")
...
Generated by agent-build relay. 自验证通过（文档结构 / node --check）。"
